...
class RegWeightedL1Loss(nn.Module):
  ...
  def forward(self, output, mask, ind, target, depth_adjustment_type, dep, dep_mask, m=100, factor=20):
    pred = _tranpose_and_gather_feat(output, ind)

    loss = F.l1_loss(pred * mask, target * mask, reduction='none').sum(axis=2)

    if depth_adjustment_type:
      batch_size = output.shape[0]
      K = pred.shape[1]
      gt_dep = (dep * dep_mask).view(batch_size, K)
      if depth_adjustment_type == 'linear':
        gt_dep = 1 + gt_dep * (factor-1)/m
      elif depth_adjustment_type == 'exp':
        gt_dep = torch.exp(math.log(factor)*gt_dep/m)
      elif depth_adjustment_type == 'quad':
        gt_dep = 1 + (gt_dep**2) * (factor-1)/(m**2)
      elif depth_adjustment_type == 'log':
        gt_dep = 1 + torch.log(1+gt_dep)*((factor-1)/math.log(1+m))
      loss = loss * gt_dep
...
class DepthLoss(nn.Module):
  ...
  def forward(self, output, target, ind, mask, cat, depth_adjustment_type, dep, dep_mask, m=100, factor=20):
    '''
    Arguments:
      out, target: B x C x H x W
      ind, mask: B x M
    ...
    '''
    print(f'pred is of size {pred.shape[2]}')
    will equal 1
    '''
    if pred.shape[2] > 1:
